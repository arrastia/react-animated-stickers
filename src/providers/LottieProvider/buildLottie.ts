...
export const buildRLottieObject = function() {
  let rlottie: any = {},
    apiInitStarted = false,
    apiInited = false,
    initCallbacks: any = [];
  let deviceRatio = window.devicePixelRatio || 1;
  let rlottieWorkers: any = [],
    curWorkerNum = 0,
    rlottieFrames = new Map();
    ...
    if (apiInited) {
      ...
      if (!apiInitStarted) {
        ...
        for (let workerNum = 0; workerNum < rlottie.WORKERS_LIMIT; workerNum++) {
          (function(workerNum) {
            const rlottieWorker = (rlottieWorkers[workerNum] = new QueryableWorker('./lottie/lottie-wasm.worker.js'));
  ...
  function initPlayer(el: any, options: any) {
    ...
    options = options || {};
    const rlPlayer: any = (el.rlPlayer = {});
  ...
  function render(rlPlayer: any, checkViewport: any, shift: any) {
    const data = rlottieFrames.get(rlPlayer.url);
    ...
    const frame = shift ? data.frameQueue.shift() : data.frameQueue.queue.length > 0 ? data.frameQueue.queue[0] : null;
  ...
  function doRender(rlPlayer: any, frame: any) {
  ...
  function requestFrame(reqId: any, frameNo: any) {
  ...
  function onFrame(reqId: any, frameNo: any, frame: any) {
    const rlPlayer = rlottie.players[reqId];
    const data = rlottieFrames.get(rlPlayer.url);

    if (rlPlayer.options.cachingModulo && !data.frames[frameNo] && (!frameNo || (reqId + frameNo) % rlPlayer.options.cachingModulo)) {
  ...
  rlottie.init = function(el: any, options: any) {
  ...
  rlottie.destroy = function(el: any) {
    destroyPlayer(el);
  };
...
class QueryableWorker {
  ...
  sendQuery(queryMethod: any) {
    ...
    if (buildRLottieObject().isSafari) {
